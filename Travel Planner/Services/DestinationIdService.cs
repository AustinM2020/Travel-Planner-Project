...
namespace Travel_Planner.Services
{
    public class DestinationIdService
    {
        public async Task<DestinationInfo> GetDestinationId(Vacation vacation)
        {
            ...
            HttpResponseMessage response = await client.GetAsync($"https://tripadvisor1.p.rapidapi.com/locations/search?location_id=1&limit=30&sort=relevance&offset=0&lang=en_US&currency=USD&units=km&query={vacation.Destination}");
